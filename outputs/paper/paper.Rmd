...
According to the experiment, there are six treatment conditions including deepfake video, audio, text, skit, campaign attack ad, and control. During the experiment, informational contents are randomly paired to one of six conditions. From Figure \@ref(fig:treat) , we can see that the six types of treatments are equally sampled. In other words, in the process of exposure, all media have the same chance of being viewed. As a result, it eliminates the impact of alternative characteristics on the final results.
...
Figure \@ref(fig:sexism) uses the jitter plot to visualize the distribution of sexism levels by partisanship. As we can see from the chart, people’s sexism level is approximately at level 3. In particular, voters in the Republican party have the most number of  sexists, which also explains one potential reason why their support rate is relatively lower for Warren. Next comes the Independent party. In contrast, respondents who identify as Democrat party are less likely to have prejudice or discrimination based on a person's sex or gender. According to the Figure \@ref(fig:sexism), people in the Democratic party are deem to hold the fairest attitude about sexism. 
...
s1<-
  ggplot(EDA, aes(x = PID, y= ambivalent_sexism,color=PID)) +
  geom_point(position = "jitter", alpha = 1/5)+
  labs(title="Ambivalent sexism level by partisanship", x="Political Party", y="Sexism Level")+
  theme_light() + 
  scale_color_manual(name = "Political Parties", values = c("#DB7558", "#FFCF57","#8FCDD4")) 
s1
```
...
```{r postfavor, fig.cap="Distribution of feelings towards Warren in six different situations",echo=FALSE,fig.width=5, fig.height=7}
...
# Model 
[@citePaper] paper used a multiple regression model to test the effect of treatment conditions towards Warren Feeling Thermometer, which are randomly paired informational messages about deepfake to one of six conditions. The six conditions included video, audio, text, skit, campaign attack ad, and control which is no clip. Based on Munger's model, we get inspirations to reinforce the model. Firstly, we use feature selection method to go through all the dataset variables. It helps us find out all the potential influential features besides the six excrement treatments. After we remove the low feature importance variables, we rebuild the multiple linear regression model. The detailed process are illustrate in the following sub-sections. 
...
$$\hat Y_{Sentiment\ Score}=\hat \beta_0+\sum_{a=male}^{other}\hat \beta_1\cdot Gender_a+\sum_{b=High\ school}^{Postgraduate}\hat \beta_2\cdot educ_b+\sum_{c=25K\ -49K}^{>150K}\hat \beta_3\cdot HHI_c+\sum_{d=white}^{black}\hat \beta_4\cdot Ethnicity_d$$
...
m1 %>%
  ...
  mutate_if(is.numeric, format, digits=3)%>%  #change significant figure
...
Similarly, the same analysis has been done for the rest unpaired two-sample t-tests. In our study, we used a 5% significance level. Table \@ref(tab:attest2) shows the test result of how deepfake videos and texts impact audiences' feelings. The result shows that the difference in affect level between video (Mean = 41.3) and text (Mean=44.2) was not significant given the p-value is greater than 0.05.
...
Table \@ref(tab:attest3) shows the test result of how deepfake videos and audios impact audiences' feelings. The difference in affect level between video(Mean = 41.3) and audio (Mean=43.9) was not significant given the p-value is greater than 0.05.
...
Table \@ref(tab:attest4) shows the test result of how deepfake videos and skit impact audiences feeling. The difference in affect level between video(Mean = 41.3) and skit (Mean=43) was not significant given the p-value is greater than 0.05.
...
This paper replicates the first experiment of [@citePaper] to further study the persuasion power of deepfake through the channel of its affective manipulation on viewer’s political perception. Media heterogeneity effects are reviewed via t-tests between other false media and deepfake.  The heterogeneity in affective response by viewer traits is examined with a multiple linear regression model. Feature selection is performed via the Random Forest classifier to select the top 20 relevant variables into our model input, which identified additional important socioeconomic features other than the ones from the original paper. 
...
Some modelling bias and ethical concerns in our model and the original paper are addressed:

### Privacy problem on zip code
To protect the privacy of respondents in the experiment, detailed address information such as zip code was excluded from our linear regression model. Instead, location was proxied by region. Thereby, it would be impossible to reverse engineer the identity of the respondent by any ill-intended third parties. Furthermore, postal code is a proxy for socioeconomic status, which correlates with HHI (household income), causing multicollinearity and eventually biasing the coefficient [@citeCorrelation].


### Choice of candidate - election result disruption & pre-existing gender prejudice
In the original paper, the one-month experiment was run prior to the 2020 U.S. presidential election, to minimize the risk of influencing the election outcome and in the meantime to have discernible effect for the experiment, a high-profile candidate who was not finalisted to be the democratic nominee should be selected. In addition, [@citePaper] would like to test for the impact of pre-existing sexism against women among subjects on the effect of the deepfake. Elizabeth Warren came naturally to be the choice of the candidate in the [@citePaper] paper who meets all the conditions above.


### Active debrief - post-experiment potential change in participants’ political behavior
At the completion of the survey by [@citePaper], an extensive debrief was conducted on all subjects which instructed respondents to affirm that the treatment media about Elizabeth Warren was false. The active debrief was done to educate participants on deepfake media. And thus it would be unlikely for the treated group to change their political behavior later on as a post-exposure consequence.
...
It is worth our attention that compared to political knowledge, internet usage has a more significant effect on respondents’ affective impression. Perhaps that the more frequent interest 
As for the regression model result on affective response towards Elizabeth Warren, heterogeneous effects in respondents from different socioeconomic groups were observed, which are in line with the findings by [@citePaper]. All else held constant, on average people with a postgraduate degree significantly favor Warren by 9.3 points more than people with lower than high school education. High-incomers with annual income above $100k favor Warren by 4 points more than people who earn less than $25,000 a year. In contrast, people who were unwilling to disclose their salaries disliked the candidate by 4.94 points compared to the low-incomers. Elderly people aged over 65 deduct feeling marks by 4.35 points towards Warren after being exposed to the deepfake media about Warren. [@citePaper] finds that on average postgraduates significantly favor Warren by 12.71 points more and elderly people dislike Warren more by 5.31 points than people younger than 65. The difference is that we include a new feature of income (HHI) which shares some of the variation in affective level with education compared with what [@citePaper] finds.

### Impact from partisan motivated reasoning and sexism discrimination
Partisanship accounts for a large part of the sentiment score towards Warren. For people supporting rival parties to the Democratic as the Republican and Independent, they substantially dislike Warren by about 40 and 27 points on average respectively compared with people identifying with the Democratic. The coefficient results echo with [@citePaper]’s result of -39 and -27.5. Additionally, the map would also justify the positive coefficient for the Northeast region, where Warren is from; she is home based in Massachusetts [@citeWarrenBase]. The effect of regions is not accounted for by [@citePaper]’s paper. For people holding ambivalent sexism views against women, a higher level in their hostile attitude would deduct 4 more marks from their feeling scales for the female candidate Warren on average, roughly the same as -3.6 by [@citePaper]’s result. 

### Deepfake and other false media, not that different in fooling the public
Although there is significant influence from the deepfake video on altering people’s sentiment towards the candidate, the two-sample t-test results between deepfake and other false media assure us that the its degree of detriment is no different from false audio or text in terms of deception and affective impression. Our conclusion is consistent with [@citePaper]’s. 

### Political knowledge and internet usage
It is worth our attention that compared to political knowledge, internet usage has a more significant effect on respondents’ affective impression. On average, one unit increase in interest usage frequency will increase viewer’s affective score on Warren by 1.12, matching with [@citePaper]’s 1.30.  Perhaps that the more frequent interest users gain a more holistic understanding 
...
## Appendix A 
All the Missing value in the dataset. 
